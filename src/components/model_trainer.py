...
class ModelTrainer:
    ...
    def initiate_model_trainer(self,
        ...
        try:
            ...
            # Copy model to project root for local offline prediction
            try:
                import shutil
                shutil.copy(self.model_trainer_config.trained_model_path, "model.pkl")
                logging.info("Saved trained model to project root as model.pkl")
            except Exception as e:
                logging.warning(f"Failed to copy model to project root: {e}")

            try:
                self.utils.upload_file(from_filename=self.model_trainer_config.trained_model_path,
                                       to_filename="model.pkl",
                                       bucket_name=AWS_S3_BUCKET_NAME)
                logging.info("Uploaded model to AWS S3 bucket.")
            except Exception as e:
                logging.warning(f"Could not upload model to AWS S3 (running in local mode): {e}")
